package ro.feedershop.html.util;

/**
 * Created by nevastuica on 7/27/2014.
 */
public class ConstanteUtil {

    //    parsing indexes
    static final String NUME_CLIENT = "Nume client";
    static final String GRUP_CLIENT = "Grup de clienţi";
    static final String ADRESA_FACTURARE = "Adresă de facturareEditare";
    static final String ADRESA_LIVRARE = "Adresa de livrareEditare";
    static final String INFO_PLATA = "Informaţii plată";
    static final String PROD_START = "Rând total";
    static final String PROD_END = "Istoric comentarii";
    static final String COMANDA_START = "Comanda #";
    static final String COMANDA_END = "Cumpărat de la";
    static final String COD_INTERN = "Cod intern produs:";
}
